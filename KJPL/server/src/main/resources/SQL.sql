...
CREATE TABLE PAYROLL.RATE (RATE_ID INT NOT NULL GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1),
			CURR_DATE DATE NOT NULL, PERMANENT DOUBLE, SPECIAL_BADLY DOUBLE, BADLY DOUBLE, LEARNER DOUBLE, SEMI_SKILLED DOUBLE, NEW_ENTRANCE DOUBLE,
       		OUTSIDER DOUBLE, OTHER_MILL DOUBLE, VOUCHER DOUBLE, 
       		FNG_SWG DOUBLE,
			AGENCY_DURWAN DOUBLE,
			STAFF DOUBLE,
			JUTE_SELECTION DOUBLE,
			STATUS VARCHAR(6), 
       CREATED_BY VARCHAR(20), CREATED_DATE TIMESTAMP, UPDATED_BY VARCHAR(20), UPDATE_DATE TIMESTAMP,
       UNIQUE (CURR_DATE),
       PRIMARY KEY(RATE_ID));
...
update payroll.DEPT set dept_code='0505' where 
